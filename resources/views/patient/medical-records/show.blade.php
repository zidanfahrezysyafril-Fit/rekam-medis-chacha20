<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center w-full">
            ...
            <a href="{{ route('patient.medical-records.pdf', $medicalRecord->id) }}" class="inline-flex items-center px-4 py-2 bg-green-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-green-700 active:bg-green-900 focus:outline-none focus:border-green-900 focus:ring ring-green-300 disabled:opacity-25 transition ease-in-out duration-150">
                Download PDF
            </a>
        </div>
    </x-slot>
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden mb-6">
        ...
        <div class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    <p class="text-sm text-gray-500 font-medium">Attending Doctor</p>
                    <p class="text-base text-gray-900">Dr. {{ $medicalRecord->doctor->doctor_name }} ({{ $medicalRecord->doctor->specialization }})</p>
                </div>
            </div>
            ...
            <div class="mt-6 text-right">
                <p class="text-xs text-gray-400 italic">This data was securely decrypted using ChaCha20 just for you.</p>
            </div>
        </div>
    </div>
    
    <div class="mb-4">
        <a href="{{ route('patient.medical-records.index') }}" class="text-blue-600 hover:text-blue-800 font-medium text-sm">
